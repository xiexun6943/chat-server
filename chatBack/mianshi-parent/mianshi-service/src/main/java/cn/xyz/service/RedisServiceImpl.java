...
public class RedisServiceImpl {
	...
	public static final String USER_OPERATION_IP="user_operation_ip:%s";
	...
	**/ 
	...
	/** @Description: 用户登录最新ip记录
	 * @param userId
	 * @param userOperationLog
	 **/
	public void savegetUserOperatioIp(Integer userId, UserOperationLog userOperationLog){
		String key = String.format(USER_OPERATION_IP,userId);
		RBucket<Object> bucket = redissonClient.getBucket(key);
		bucket.delete();
		bucket.set(userOperationLog);
		bucket.expire(KConstants.Expire.DAY7, TimeUnit.SECONDS);
	}

	public UserOperationLog getUserOperatioIp(Integer userId){
		String key = String.format(USER_OPERATION_IP, userId);
		RBucket<Object> bucket = redissonClient.getBucket(key);
		UserOperationLog userOperationLog = (UserOperationLog) bucket.get();
