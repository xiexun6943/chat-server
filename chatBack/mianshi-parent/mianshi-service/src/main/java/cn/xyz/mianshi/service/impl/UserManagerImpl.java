...
//			String _md5_md5 = DigestUtils.md5Hex(_md5);
	...
	@Override
	public Map<String, Object> login(UserExample example, HttpServletRequest request) {
		User user =null;
		...
		if (null == user) {
			...
			return loginSuccess(user, example,request);
		}

	}

	//登陆成功方法
	public  Map<String, Object> loginSuccess(User user,UserExample example,HttpServletRequest request){
		...
		// 保存登录日志
//		getUserRepository().saveUserOperationLog(user,example,request, ImConstants.LOG_LOGIN);
		...
		//记录用户登陆的ip信息
		SKBeanUtils.getUserOperationLogManager().saveUserOperationLog(user,request,2);
		...
		//新加用户状态
		SKBeanUtils.getRedisService().saveUserOnline(user.getUserId().toString(), 1);
		SKBeanUtils.getUserManager().updateAttribute(user.getUserId(),"onlinestate", 1);
		return data;
	}
	...
	public void logout(String access_token,String areaCode,String userKey,String devicekey) {
		...
		//新加用户状态
		SKBeanUtils.getRedisService().saveUserOnline(ReqUtil.getUserId().toString(), 0);
		SKBeanUtils.getUserManager().updateAttribute(ReqUtil.getUserId(),"onlinestate", 0);
//		System.out.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>注销更新用户状态"+ReqUtil.getUserId());
	}
	...
	@Override
	public Map<String, Object> registerIMUser(UserExample example,HttpServletRequest request) throws Exception{
		...
		if (null != data) {
			...
			SKBeanUtils.getUserOperationLogManager().saveUserRegisterLog(example,request,1);
			return data;
		}
		throw new ServiceException("用户注册失败");
	}
...
//					logger.error(q.toString());
	...
	**/
	 ...
	 * @return
	 **/
	public boolean isBatchOpenMultipleDevices(Integer userId,Integer multipleUserId){
		UserSettings settings = getUser(0 != multipleUserId ? multipleUserId : userId).getSettings();
		if (null == settings) settings = new UserSettings();
		return 1 == settings.getMultipleDevices() ? true : false;
	}
	...
	**/
	...
	public void autoBatchAddUsers(List<User> userList,int reqUid) {

		ThreadUtil.executeInThread(new Callback() {
			@Override
			public void execute(Object obj) {
				batchAddUsers(userList,reqUid);
			}
		});
	}

	public List<Integer> batchAddUsers(List<User> userList,int reqUid){
		Random rand = new Random();
		List<Integer> userIds = new ArrayList<Integer>();
		UserExample  userExample= new UserExample();
		//3=机器账号，由系统自动生成
...
//					RandomUtil.getRandomEnAndNum(rand.nextInt(4)+2);
			...
			userExample.setPassword(DigestUtils.md5Hex(userList.get(i).getPassword()));
			userExample.setTelephone("86"+userList.get(i).getPhone());
			userExample.setPhone(userList.get(i).getPhone());
			userExample.setName(userList.get(i).getNickname());
			userExample.setNickname(userList.get(i).getNickname());
			userExample.setDescription(String.valueOf(userId));
			userExample.setSex(userList.get(i).getSex());
			if(userId!=0 && getUserRepository().addUser(userId, userExample)!=null){
				try {
					KXMPPServiceImpl.getInstance().registerAndXmppVersion(userId.toString(), userExample.getPassword());
					System.out.println("第"+(i+1)+"条用户"+userExample.getNickname()+"数据已经生成");
					SKBeanUtils.getFriendsManager().followUser(userId, 10000, 0);
					// 调用组织架构功能示例方法
					SKBeanUtils.getCompanyManager().autoJoinCompany(userId);
					// 默认成为好友
					defaultTelephones(userExample, userId);
					// 自动创建 好友标签
					SKBeanUtils.getFriendGroupManager().autoBatchCreateGroup(userId,reqUid);
					if(userExample.getUserType()!=null){
						if(userExample.getUserType()==3){
							SKBeanUtils.getRoomManager().join(userId, new ObjectId("5a2606854adfdc0cd071485e"),3);
						}
					}
					// 清除redis中没有系统号的表
					SKBeanUtils.getRedisService().deleteNoSystemNumUserIds();
					// 清除短信验证码
					SKBeanUtils.getSMSService().deleteSMSCode(userExample.getTelephone());
		...
		System.out.println("批量注册用户"+userIds.toString()+"总共为:"+userIds.size()+"个");
		return userIds;
	}
