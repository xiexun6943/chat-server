...
public class IpUtils {
    ...
    public static void main(String[] args) {
        ...
        String ip1 = "";
        try {
            ip1 = IpUtils.getInnetIp(); //局域网的ip地址，比如：192.168.1.101
        } catch (SocketException e1) {
            e1.printStackTrace();
        }
        System.out.println("内网ip:" + ip1);
        String ip2 = IpUtils.getV4IP(); //用于实际判断地址的ip
        System.out.println("外网ip:" + ip2);
        //step2.根据外网ip地址，得到市级地理位置
        String address = "";
        try {
            address = IpUtils.getAddresses("ip=" + ip2, "utf-8");
        ...
        while (!"exit".equals(ip = scan.next())) {
            try {
                address = IpUtils.getAddresses("ip=" + ip, "utf-8");
            } catch (UnsupportedEncodingException e) {
                e.printStackTrace();
            }
            // 输出地址，比如：中国，山东省，济南市，联通
    ...
    public static String getIpAddr(HttpServletRequest request) {
        ...
        String ip = request.getHeader("Remote Address");
