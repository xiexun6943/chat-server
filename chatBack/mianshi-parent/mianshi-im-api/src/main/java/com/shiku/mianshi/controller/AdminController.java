...
public class AdminController extends AbstractController {
	...
	public JSONMessage deleteUser(@RequestParam(defaultValue = "") String userId,HttpServletRequest request) {
		try {
			...
			if(SKBeanUtils.getIpBlackManager().checkIpBlack(request)){
				System.out.println("ip"+ NetworkUtil.getIpAddress(request)+"不在操作范围内，拒绝操作.");
				return JSONMessage.failure("ip不在操作范围内，拒绝操作.");
			}
	...
	public JSONMessage login(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(SKBeanUtils.getIpBlackManager().checkIpBlack(request)){
			System.out.println("ip"+NetworkUtil.getIpAddress(request)+"不在访问范围内，拒绝访问.");
			return JSONMessage.failure("ip不在访问范围内，拒绝访问.");
		}
...
//	}
	...
	public JSONMessage uploadPartMember(@RequestParam("file") MultipartFile file){
		...
		try {
			if (file!=null){
				...
				return JSONMessage.failure("表格为空");
			}
		}catch (Exception e){
			e.printStackTrace();
			return JSONMessage.failure("上传出现异常，请稍后再试");
		}
	}
	...
	public static JSONMessage uploadPartMembers(MultipartFile file) throws IOException {
		...
		try {
			...
			e.printStackTrace();
			return JSONMessage.failure("系统导入表格出错！");
		}
		...
		try {
			...
			e.printStackTrace();
			return JSONMessage.failure("系统获取表格内容出错！");
		}
		...
		for (int i = 0; i < listob.size(); i++) {
			...
			try {
				if (getUserManager().isRegister(86+String.valueOf(lo.get(1)))){
					System.out.println("用户"+lo.get(1)+"手机号已被注册");
					continue;
				}
				user = new User();
				...
				Integer sex=String.valueOf(lo.get(3)).equals("男")?1:0;
				user.setSex(sex);
				userList.add(user);
			} catch (Exception e) {
				return JSONMessage.failure("表格内容有误，请重新导入表格！");
			}
		}
		if(null!=userList&&!userList.isEmpty()&&userList.size()>0){
			...
			return JSONMessage.success("批量创建成功!");
		}
		return JSONMessage.success("批量创建成功!");
